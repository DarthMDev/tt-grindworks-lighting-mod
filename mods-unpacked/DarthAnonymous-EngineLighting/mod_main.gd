...
const MOD_DIR := "DarthAnonymous-EngineLighting" # Name of the directory that this file is in
const LOG_NAME := "DarthAnonymous-EngineLighting:Main" # Full ID of the mod (AuthorName-ModName)
...
func add_persistent_sun() -> void:
    # make sure the sun doesn't already exist
    var root = get_tree().root
    print("Children in root: ", root.get_children())
    if not root.has_node("Sun"):
        var sun = DirectionalLight3D.new()
        sun.name = "Sun"
        sun.rotation_degrees = Vector3(-65, 150, 0)
        sun.shadow_enabled = true
        sun.light_energy = 1.0
        sun.light_color = Color(1, 1, 1)  # Pure white light
        # position the sun high in the sky
        sun.sky_mode = DirectionalLight3D.SKY_MODE_LIGHT_AND_SKY
        
        root.add_child.call_deferred(sun)
        print('sun added')
        print("Sun visibility: ", sun.visible)
        print("Sun shadow: ", sun.shadow_enabled)
        print("Sun energy: ", sun.light_energy)
        print("Sky mode: ", sun.sky_mode)
    else:
